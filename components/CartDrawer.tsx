...
export default function CartDrawer({ isOpen, onClose }: CartDrawerProps) {
  ...
  const { cart = [], removeFromCart = () => {}, addToCart = () => {}, total = 0 } = cartContext;
            ...
            cart.map((item: any) => (
              <div key={item.id} className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm group">
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <button
                        ...
                        className="w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center font-black text-slate-600 transition-colors"
                      >
                        −
                      </button>
                      <span className="w-6 text-center font-black text-slate-800 text-sm">{item.cantidad || 1}</span>
                      <button
                        onClick={() => addToCart(item)}
                        className="w-7 h-7 rounded-lg bg-slate-100 hover:bg-blue-600 hover:text-white flex items-center justify-center font-black text-slate-600 transition-colors"
                      >
                        +
                      </button>
                    </div>
                  ...
                  <button
                    onClick={() => { for(let i = 0; i < (item.cantidad || 1); i++) removeFromCart(item.id); }}
