...
import { User as UserIcon, Phone, MapPin, Search, ArrowLeft, Mail, Trash2, Edit, Save, X, Eye, EyeOff, Lock } from 'lucide-react';
...
const inputS = { padding: '12px', borderRadius: '12px', border: '1px solid #e5e7eb', outline: 'none', fontSize: '14px' };
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f9fafb', padding: '40px 20px' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
          <h1 style={{ fontSize: '32px', fontWeight: 900, margin: 0 }}>Gestión de Clientes</h1>
          <div style={{ color: '#6b7280', fontSize: '14px' }}>{clientes.length} clientes registrados</div>
        </div>

        <div style={{ position: 'relative', marginBottom: '30px' }}>
          <Search style={{ position: 'absolute', left: '15px', top: '12px', color: '#9ca3af' }} size={20} />
          <input 
            style={{ width: '100%', padding: '12px 12px 12px 45px', borderRadius: '15px', border: '1px solid #e5e7eb', fontSize: '16px' }} 
        ...
        <div style={{ ...cardS, marginBottom: '40px', border: '2px dashed #e5e7eb', backgroundColor: '#fff' }}>
          <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 'bold' }}>Registrar Nuevo Cliente</h3>
          <form onSubmit={crearCliente} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
            ...
            <button type="submit" disabled={creating} style={{ ...btnPrimaryS, gridColumn: 'span 2' }}>
        ...
        {cargando ? (
          <div style={{ textAlign: 'center', padding: '50px' }}>Cargando clientes...</div>
        ) : (
